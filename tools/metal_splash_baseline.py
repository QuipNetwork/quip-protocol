...
def metal_splash_baseline_test(
    ...
    """Test Metal Splash performance with baseline format and evaluation logic.

    Args:
        timeout_minutes: Test timeout in minutes
        output_file: Path to save JSON results
        only_label: Run only specific config (e.g., "Light Splash")
        ...
        max_treewidth: Maximum junction tree treewidth (default: 4)
        max_splash_size: Maximum variables per Splash (default: 64)
    """
    if h_values is None:
        h_values = [-1.0, 0.0, 1.0]

    print(f"Metal Splash Baseline Parameter Test")
    print("=" * 50)
    print(f"Timeout: {timeout_minutes} minutes")
    print(f"h_values: {h_values}")
    print(f"max_treewidth: {max_treewidth}")
    print(f"max_splash_size: {max_splash_size}")

    # Initialize sampler
    try:
        splash_sampler = MetalSplashSampler(
            max_treewidth=max_treewidth,
            max_splash_size=max_splash_size
        )
        print(f"Metal Splash sampler ready")
    except Exception as e:
        print(f"Metal Splash sampler failed: {e}")
        return None
    ...
    if topology:
        ...
        print(f"Using default topology (Advantage2_system1.11)")
    ...
    test_configs = [
        (256, 64, "Light Splash"),
        (512, 100, "Low Splash"),
        (1024, 100, "Medium Splash"),
        (2048, 150, "High Splash"),
        (4096, 200, "Very High Splash"),
        (8192, 200, "Max Splash")
    ]
    ...
    for idx, (sweeps, reads, desc) in enumerate(test_configs):
        ...
        try:
            ...
            samplesets = splash_sampler.sample_ising(
                h=h_list, J=J_list,
                num_reads=reads,
                num_sweeps=sweeps
            )
...
def main():
    ...
    parser.add_argument(
        '--topology',
        type=str,
        help='Topology: Z(9,2), Advantage2_system1.11, file path, or *.embed.json.gz for embedded. '
             'Default: Advantage2_system1.11'
    )
    parser.add_argument(
        '--max-treewidth',
        type=int,
        default=4,
        help='Maximum junction tree treewidth (default: 4)'
    )
    parser.add_argument(
        '--max-splash-size',
        type=int,
        default=64,
        help='Maximum variables per Splash (default: 64)'
    )
    ...
    if not output_file:
        ...
        output_file = f"metal_splash_baseline_results_{timestamp}.json"

    # Run test
    metal_splash_baseline_test(
        ...
        max_splash_size=args.max_splash_size
    )

    print(f"\nMetal Splash baseline test complete!")
