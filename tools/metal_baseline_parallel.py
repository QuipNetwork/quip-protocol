...
# Optional: new parallel sampler alias (use the same unified sampler)
try:
    from GPU.metal_sampler_parallel import MetalKernelDimodSampler as MetalSamplerParallel
...
def metal_baseline_test(timeout_minutes=10.0, output_file=None, num_replicas=None, swap_interval=15, T_min=0.1, T_max=5.0, large_problem=False, only_label=None, sampler_choice: str = "original"):
    ...
    if sampler_choice == "parallel":
        if not PARALLEL_AVAILABLE:
            ...
            return None
        try:
            metal_sampler = MetalSamplerParallel("mps")
        ...
        if METAL_AVAILABLE:
            try:
                metal_sampler = MetalKernelDimodSampler(verbose=False)
                # Get nodes and edges from the topology (same as original)
                topology_graph = DEFAULT_TOPOLOGY.graph
                nodes = list(topology_graph.nodes())
                edges = list(topology_graph.edges())
                sampler_type = "metal-parallel"
                print("✅ Metal Parallel Tempering sampler ready (parallel kernel)")
            except Exception as e:
    ...
    for sweeps, reads, desc in test_configs:
        ...
        try:
            ...
            sampleset = metal_sampler.sample_ising(
                ...
                T_min=T_min,
                T_max=T_max,
                sample_interval=None  # Let sampler choose
            )
...
def main():
    ...
    if not output_file:
        ...
        output_file = f"metal_parallel_baseline_results_{timestamp}.json"
