...
def _build_profile_json(data, clock_khz, kernel_name,
    ...
    # Topology info for metrics panel
    topo = get_topology_stats()
...
def _render_flame_svg(bars, total_width_px=700,
                       bar_height=32, pad=2):
    """Build inline SVG string from bars JSON data.

    Each bar gets data attributes for JS interaction.
    """
    max_row = max(b["row"] for b in bars)
    svg_h = (max_row + 1) * (bar_height + pad) + pad
    svg_w = total_width_px

    parts = [
        f'<svg id="flame-svg" width="{svg_w}" '
        f'height="{svg_h}" '
    ...
    for b in bars:
        x = b["x"] * svg_w
        w = b["w"] * svg_w
        # Invert rows so deepest is at top (flame style)
        y = ((max_row - b["row"])
             * (bar_height + pad) + pad)
        ...
        # Label text (skip if bar too narrow)
        if w > 40:
            label = b["name"]
            if w > 120:
                label += f" ({b['pct']}%)"
...
.nu{color:#d19a66}
.flame-bar{cursor:pointer;opacity:0.85;
stroke:#1a1a2e;stroke-width:1;
transition:opacity .15s}
.flame-bar:hover{opacity:1;stroke:#fff;
stroke-width:2}
.flame-bar.hl{opacity:1;stroke:#fff;
stroke-width:2}
.flame-bar.selected{opacity:1;stroke:#e94560;
stroke-width:3}
#tooltip{position:fixed;background:#16213e;
border:1px solid #e94560;border-radius:6px;
padding:8px 12px;pointer-events:none;
display:none;z-index:100;font-size:12px;
max-width:300px;box-shadow:0 4px 12px rgba(0,0,0,.5)}
#tooltip .tt-name{color:#e94560;font-weight:bold;
font-size:14px}
#tooltip .tt-row{margin-top:4px}
#tooltip .tt-label{color:#888}
#tooltip .tt-val{color:#e0e0e0;margin-left:6px}
...
Click a bar to highlight only its direct source lines
(excluding children). Click background to clear.
</div>
</div>
...
var bars = document.querySelectorAll(".flame-bar");
...
function clearHighlights() {
  for (var i = 0; i < bars.length; i++) {
    bars[i].classList.remove("hl","selected");
  }
...
function setTooltip(e, bar) {
  var name = document.createElement("div");
  name.className = "tt-name";
  name.textContent = bar.dataset.name;
  var mkRow = function(label, val) {
    var row = document.createElement("div");
    row.className = "tt-row";
    var sp1 = document.createElement("span");
    sp1.className = "tt-label";
    sp1.textContent = label;
    var sp2 = document.createElement("span");
    sp2.className = "tt-val";
    sp2.textContent = val;
    row.appendChild(sp1);
    row.appendChild(sp2);
    return row;
  ...
  tooltip.appendChild(mkRow("Cycles: ",
    parseInt(bar.dataset.cycles, 10).toLocaleString()));
  tooltip.appendChild(mkRow("Time: ",
    bar.dataset.time));
  tooltip.appendChild(mkRow("Share: ",
    bar.dataset.pct + "%"));
...
for (var i = 0; i < bars.length; i++) {
  (function(bar) {
    ...
    bar.addEventListener("mouseleave", function() {
      if (!selectedBar) clearHighlights();
    ...
    if args.kernel == "gibbs":
        ...
        if args.flame_html:
            generate_flamegraph_html(
                ...
                args.sweeps, gpu_name,
                "flamegraphs/flamegraph_gibbs.html",
            )
    else:
        ...
        if args.flame_html:
            generate_flamegraph_html(
                data, clock_khz, "sa",
                args.sweeps, gpu_name,
                "flamegraphs/flamegraph_sa.html",
            )
...
# Region index -> (start_line, end_line) in .cu source
GIBBS_SOURCE_MAP = {
    0: (319, 434),  # ANNEALING_TOTAL
    1: (320, 421),  # BETA_ITER_TOTAL
    2: (326, 413),  # SWEEP_ITER_TOTAL
    3: (330, 406),  # COLOR_ITER_TOTAL
    4: (335, 344),  # COLOR_SETUP
    5: (347, 394),  # NODE_LOOP_TOTAL
    6: (357, 369),  # FIELD_COMPUTE
    7: (371, 389),  # SPIN_UPDATE
    8: (396, 405),  # SYNC_COLOR
}

SA_SOURCE_MAP = {
    0: (439, 527),  # SA_TOTAL
    1: (445, 449),  # BETA_OVERHEAD
    2: (454, 521),  # SWEEP_TOTAL
    4: (459, 463),  # THRESHOLD_SKIP
    5: (471, 482),  # ACCEPT_DECIDE
    6: (487, 515),  # FLIP_TOTAL
    7: (499, 508),  # NEIGHBOR_LOOP
}
