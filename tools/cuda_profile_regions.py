...
# These map to cuda_gibbs_self_feeding in cuda_gibbs.cu
GIBBS_SOURCE_MAP = {
    0: (385, 487),  # ANNEALING_TOTAL
    1: (389, 481),  # BETA_ITER_TOTAL
    2: (396, 474),  # SWEEP_ITER_TOTAL
    3: (401, 468),  # COLOR_ITER_TOTAL
    4: (403, 410),  # COLOR_SETUP
    5: (412, 462),  # NODE_LOOP_TOTAL
    6: (420, 435),  # FIELD_COMPUTE
    7: (440, 456),  # SPIN_UPDATE
    8: (464, 468),  # SYNC_COLOR
}
...
def main():
    ...
    print_precise_ticks(
        data, clock_khz, args.sweeps, kernel, gpu_name,
    )

    # Generate outputs
    region_names = (GIBBS_REGION_NAMES
                    if kernel == "gibbs"
                    else SA_REGION_NAMES)
    if args.plot:
        plot_profile(
            data, region_names, clock_khz,
            kernel, f"profile_{kernel}.png",
        )
    if args.flame:
        generate_flamegraph(
            data, clock_khz, kernel,
