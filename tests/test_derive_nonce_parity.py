...
``quip-protocol-rs`` sibling checkout. Post-MR-!20 the fixture schema is:
  ...
  - ``expected_nonce_hex``: 32-byte hex (full BLAKE3 digest, not the
                           legacy truncated u64)
...
def test_derive_nonce_accepts_zero():
    out = derive_nonce(b"\x00" * 32, b"\x00" * 32, 0, b"\x00" * 32)
