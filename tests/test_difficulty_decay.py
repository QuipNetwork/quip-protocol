...
# ----------------------------------------------------------------------
# block_when_energy_clears — anticipatory-submission decay-block predictor
# ----------------------------------------------------------------------


def test_block_when_energy_clears_returns_current_block_when_already_clears(
    test_curve, base_difficulty,
) -> None:
    """A floor already below the live threshold clears at current_block.

    base_difficulty.max_energy_milli = -2500. A candidate floor of -2600
    is strictly below it, so the chain's ``< max_energy_milli`` gate
    already passes at the current block (no decay needed).
    """
    got = block_when_energy_clears(
        floor_energy_milli=-2600,
        current_block=100,
        base_difficulty=base_difficulty,
        last_proof_block=100,
        epoch_length=10,
        curve=test_curve,
        search_limit=1000,
    )
...
def test_block_when_energy_clears_after_k_decay_steps(
    test_curve, base_difficulty,
) -> None:
    """A floor that only clears after K decay steps returns that boundary.

    Pick a floor that sits between the base threshold and the threshold
    after some decay steps, then assert the predictor lands on the first
    decay-step boundary whose threshold strictly exceeds it.
    """
    last_proof_block = 100
    epoch_length = 10
    current_block = 100

    # Find the smallest K (>=1) whose decayed threshold clears a chosen
    # floor. Choose a floor 1 milli above the base threshold so it needs
    # at least one decay step.
    floor = base_difficulty.max_energy_milli + 1  # -2499; base does NOT clear
    # Independently compute the first clearing step.
    first_step = None
    for k in range(1, 100):
        thr = apply_decay(base_difficulty, k, test_curve).max_energy_milli
        if thr > floor:
            first_step = k
            break
    assert first_step is not None
    expected_block = last_proof_block + first_step * epoch_length

    got = block_when_energy_clears(
        floor_energy_milli=floor,
    ...
    # Sanity: the chain must indeed gate-pass at that block and NOT before.
    assert current_difficulty(
        got, base_difficulty, last_proof_block, epoch_length, test_curve,
    ).max_energy_milli > floor
    assert current_difficulty(
        got - epoch_length, base_difficulty, last_proof_block, epoch_length,
        test_curve,
    ).max_energy_milli <= floor


def test_block_when_energy_clears_never_within_limit_returns_none(
    test_curve, base_difficulty,
) -> None:
    """A floor far easier than any reachable threshold never clears → None.

    The threshold eases only toward ``curve.max_milli``; a floor at or
    above that ceiling can never be strictly exceeded, so within any
    finite window the predictor returns None.
    """
    # max_milli is the easiest the threshold can ever get; a floor equal
    # to it can never be strictly exceeded.
    got = block_when_energy_clears(
        floor_energy_milli=test_curve.max_milli,
        current_block=100,
        base_difficulty=base_difficulty,
        last_proof_block=100,
        epoch_length=10,
...
def test_block_when_energy_clears_genesis_no_decay(
    test_curve, base_difficulty,
) -> None:
    """last_proof_block == 0 disables decay: only current_block can clear."""
    # Floor below base → clears immediately at current_block.
    assert block_when_energy_clears(
        floor_energy_milli=base_difficulty.max_energy_milli - 1,
        current_block=42,
        base_difficulty=base_difficulty,
        last_proof_block=0,
        epoch_length=10,
    ...
    # Floor at/above base → never clears (no decay to ease it).
    assert block_when_energy_clears(
        floor_energy_milli=base_difficulty.max_energy_milli,
        current_block=42,
        base_difficulty=base_difficulty,
        last_proof_block=0,
        epoch_length=10,
...
def test_block_when_energy_clears_zero_epoch_length_no_decay(
    test_curve, base_difficulty,
) -> None:
    """epoch_length == 0 disables decay (mirrors current_difficulty)."""
    assert block_when_energy_clears(
        floor_energy_milli=base_difficulty.max_energy_milli,
        current_block=200,
        base_difficulty=base_difficulty,
...
def test_block_when_energy_clears_none_curve_no_decay(base_difficulty) -> None:
    """A None curve disables decay; only an already-clearing floor returns."""
    assert block_when_energy_clears(
        floor_energy_milli=base_difficulty.max_energy_milli,
        current_block=200,
        base_difficulty=base_difficulty,
...
def test_block_when_energy_clears_respects_search_limit_boundary(
    test_curve, base_difficulty,
) -> None:
    """If the clearing boundary falls just past search_limit → None;
    extending the limit by one step finds it."""
    last_proof_block = 100
    epoch_length = 10
    current_block = 100
    floor = base_difficulty.max_energy_milli + 1  # needs >=1 decay step

    first_step = None
    for k in range(1, 100):
        if apply_decay(base_difficulty, k, test_curve).max_energy_milli > floor:
            first_step = k
            break
    assert first_step is not None
    clearing_block = last_proof_block + first_step * epoch_length

    # search_limit one block short of the clearing boundary → None.
    short_limit = (clearing_block - current_block) - 1
    assert block_when_energy_clears(
        floor_energy_milli=floor,
        current_block=current_block,
        base_difficulty=base_difficulty,
        ...
        epoch_length=epoch_length,
        curve=test_curve,
        search_limit=short_limit,
    ) is None

    # Exactly reaching the boundary → found.
    assert block_when_energy_clears(
        floor_energy_milli=floor,
        current_block=current_block,
        base_difficulty=base_difficulty,
        ...
        epoch_length=epoch_length,
        curve=test_curve,
        search_limit=clearing_block - current_block,
    ) == clearing_block
