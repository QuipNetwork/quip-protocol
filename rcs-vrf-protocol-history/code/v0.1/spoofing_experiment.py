...
def run_experiment():
    ...
    chi_low = 0.15
    chi_high = 2.5
    print(f"v0.1 acceptance window: [{chi_low}, {chi_high}]")
    print("=" * 95)
    print(f"{'strategy':35s} {'mean F_XEB':>12s} {'std':>8s} {'pass v0 (χ≥0.15)':>18s} {'pass v0.1 (window)':>20s}")
    print("-" * 95)
    ...
    for strategy_name in ["honest_noiseless", "honest_noisy", "uniform_random",
        ...
        mean = scores.mean()
        std = scores.std()
        # v0: one-sided lower threshold only
        pass_rate_v0 = (scores >= chi_low).mean()
        # v0.1: two-sided window check
        pass_rate_v0_1 = ((scores >= chi_low) & (scores <= chi_high)).mean()
        
        results[strategy_name] = {
            "mean": mean, "std": std,
            "pass_rate_v0": pass_rate_v0,
            "pass_rate_v0_1": pass_rate_v0_1,
        }
        print(f"{strategy_name:35s} {mean:>12.4f} {std:>8.4f} "
              f"{pass_rate_v0*100:>16.0f}% {pass_rate_v0_1*100:>18.0f}%")
