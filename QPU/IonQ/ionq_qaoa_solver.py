"""IonQ QAOA solver wrapper for quantum blockchain mining.

Analogous to ibm_qaoa_solver.py — encapsulates all QAOA-specific logic
...
Uses IonQ's trapped-ion hardware or cloud simulator via qiskit-ionq,
with AerSimulator as a local fallback when no API key is available.

Dependencies:
    pip install qiskit qiskit-aer qiskit-ionq dimod numpy scipy
"""
...
# ---------------------------------------------------------------------------
# Optimizer configurations (shared with IBM — identical QAOA math)
# ---------------------------------------------------------------------------
        ...
        if not self._done:
            try:
                ...
                if not self._process.is_alive():
                    logger.error("[IonQ-QAOA] Child process died without returning a result")
                    self._result = None
                else:
...
# ---------------------------------------------------------------------------
# IonQ backend creation helper
# ---------------------------------------------------------------------------

def _create_ionq_backend(
    ionq_token: Optional[str] = None,
    ionq_backend: str = 'aer_simulator',
) -> Any:
    """Create a Qiskit backend for IonQ execution.

    Three modes:
      1. ionq_backend='aer_simulator' (default) → local AerSimulator, no API key
      2. ionq_backend='ionq_simulator' → IonQ cloud simulator, needs API key
      3. ionq_backend='ionq_qpu' or specific QPU name → real IonQ hardware

    Args:
        ionq_token: IonQ API key.  Required for modes 2 and 3.
            Can also be set via IONQ_API_KEY environment variable.
        ionq_backend: Backend name.  'aer_simulator' for local,
            'ionq_simulator' for cloud sim, 'ionq_qpu' for hardware.

    Returns:
        A Qiskit-compatible backend object.
    """
    if ionq_backend == 'aer_simulator' or ionq_backend is None:
        logger.info("[IonQ-QAOA] Using AerSimulator (local)")
        return AerSimulator()

    # IonQ cloud backends require qiskit-ionq
    try:
        from qiskit_ionq import IonQProvider
    except ImportError:
        raise ImportError(
            "qiskit-ionq is required for IonQ cloud backends. "
    ...
    # IonQProvider will check IONQ_API_KEY env var if token is None
    provider = IonQProvider(token=ionq_token)
    backend = provider.get_backend(ionq_backend)
    logger.info(f"[IonQ-QAOA] Using IonQ backend: {ionq_backend}")
    return backend
...
def _qaoa_solve_in_process(
    ...
    multiprocessing.Process can pickle it.  A fresh IonQQAOASolverWrapper is
    constructed from ``solver_config`` inside the child process.

    Args:
        solver_config: Serializable kwargs for IonQQAOASolverWrapper.__init__.
        h: Linear biases.
        J: Quadratic biases.
...
class IonQQAOASolverWrapper:
    """Wrapper that presents (h, J) → dimod.SampleSet via QAOA on IonQ.

    Encapsulates cost-operator construction, circuit building, variational
    optimization, and result conversion.  Higher-level code (IonQQAOAMiner)
    interacts only through ``solve_ising`` / ``solve_ising_async``.

    The QAOA pipeline is identical to IBM's — only the backend differs:
    IonQ trapped-ion hardware or cloud simulator via qiskit-ionq, with
    AerSimulator as a local fallback.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            ionq_token: IonQ API key.  Ignored if backend is provided.
        ...
        if backend is not None:
            ...
            self.backend = _create_ionq_backend(ionq_token, ionq_backend)

        self.is_simulator = isinstance(self.backend, AerSimulator)
        # IonQ cloud simulator is also a simulator, but not an AerSimulator instance
        backend_name = getattr(self.backend, 'name', str(self.backend))
        if 'simulator' in backend_name.lower():
            self.is_simulator = True

        # Store IonQ credentials for _get_solver_config (child process reconstruction)
        self._ionq_token = ionq_token
        ...
        logger.info(
            f"[IonQ-QAOA] Solver ready: {len(nodes)} nodes, {len(edges)} edges, "
            f"p={p}, optimizer={optimizer}, shots={shots}, backend={backend_name}"
        )
    ...
    def _transpile(self, circuit: Any) -> Any:
        ...
        For AerSimulator, uses Qiskit's standard transpilation.
        For IonQ backends, the IonQ provider handles its own transpilation
        pipeline on the server side, but we still run local transpilation
        to validate the circuit structure.
        """
        if isinstance(self.backend, AerSimulator):
            ...
            # IonQ backends handle transpilation server-side.
            # We still decompose to basis gates locally so the circuit
            # is valid for submission.
            from qiskit import transpile
            return transpile(
                circuit, backend=self.backend, optimization_level=1,
            )
    ...
    def _optimize(
        ...
        def objective(params):
            ...
            if iteration_count % 10 == 0:
                logger.debug(
                    f"[IonQ-QAOA] Optimizer iter {iteration_count}: energy={value:.4f}"
                )
            return value
        ...
        try:
            ...
            logger.info("[IonQ-QAOA] Optimization interrupted by stop_event")
            return None

        logger.info(
            f"[IonQ-QAOA] Optimization done: {self.optimizer_name}, "
    ...
    def solve_ising(
        ...
        Returns None if interrupted via stop_event.
        ...
        # Apply per-solve parameter overrides
        ...
        logger.info("[IonQ-QAOA] Step 1/5: Building cost Hamiltonian (h,J → SparsePauliOp)")
        cost_op = self._build_cost_operator(h, J)

        # 2. Build & transpile QAOA circuit
        logger.info(f"[IonQ-QAOA] Step 2/5: Building QAOA circuit (p={p})")
        circuit = self._build_circuit(cost_op, p=p)
        logger.info("[IonQ-QAOA] Step 3/5: Transpiling circuit for backend")
        circuit = self._transpile(circuit)

        # 4. Variational optimization
        logger.info(
            f"[IonQ-QAOA] Step 4/5: Variational optimization "
        ...
        logger.info(f"[IonQ-QAOA] Step 5/5: Final sampling ({final_shots} shots)")
        ...
        logger.info(
            f"[IonQ-QAOA] Solve complete in {solve_time:.2f}s — "
    ...
    def _get_solver_config(self) -> Dict[str, Any]:
        ...
        a child process.  For AerSimulator the child creates its own
        instance.  For IonQ cloud backends, the token and backend name
        are passed so the child can connect.
        """
        return {
            'nodes': self.nodes,
            'edges': self.edges,
            'backend': None,  # child process creates its own backend
            'ionq_token': self._ionq_token,
    ...
    def solve_ising_async(
        ...
        giving it its own GIL for true parallelism.
        """
        solve_start = time.time()
