...
| [`cell_quotient_interleaved.py`](cell_quotient_interleaved.py)             | Interleaved Hamiltonian-cell DP — alternative path-walk ordering used in chain-recurrence research.                                                                                            |
...
| [`interpolation.py`](interpolation.py)                                     | 1D / 2D Lagrange interpolation + CRT combine for modular point-value DP.                                                                                                                       |
| [`sparse_interp.py`](sparse_interp.py)                                     | Sparse Lagrange interpolation helpers (when the Tutte polynomial's exponent support is sparse).                                                                                               |
| [`signed_quotient.py`](signed_quotient.py)                                  | Signed-Tutte computation on σ-quotient graphs (consumed by σ-equivariant work).                                                                                                                |
