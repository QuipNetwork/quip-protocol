"""High-level API for signed-graph Tutte polynomial on 2-fold cover quotients.

For a 2-fold cover G → G/⟨σ⟩ of graphs with monodromy χ : E(G/σ) → Z_2:
  - T_signed(G/σ, χ; x, y) computes Zaslavsky's frame-matroid Tutte
    polynomial of the signed quotient graph.
  - This polynomial captures the σ-equivariant structure of G:
    specifically, sum over σ-fixed edge subsets A ⊆ E(G) of the
    Whitney term (x-1)^{r(E)-r(A)} (y-1)^{|A|-r(A)}.

Low-level point evaluation is in `tutte/graphs/signed_elim_dp.py`.
This module wraps that with:
  - High-level construction of quotient + monodromy from (G, σ).
  - Multi-point evaluation for polynomial interpolation.
  - Bivariate Lagrange interpolation modulo prime.

Use cases:
  - Compute T_signed of a 2-fold cover quotient.
  - As a building block in σ-equivariant Tutte frameworks (e.g., Burnside
    marking) for recovering T(G) from quotient invariants.
"""
