...
Modules that are **not on any live `SynthesisEngine` / `HybridSynthesisEngine`
...
| `signed_treewidth.py` | Treewidth-based DP for the *signed* Tutte polynomial. | Superseded by the faster elimination-order signed DP (`graphs/signed_elim_dp.py` + `_signed_elim_c.py`). No live importer; the prior README claim that it "backs σ-equivariant evaluations" was stale. |
