...
| 6.9 | [Cell-Quotient Grid DP](06_5_cell_quotient_grid_dp.md)         | Cell-decomposable graphs with `(rows × cols)` grid quotient. **Engine step 7.45 streamed path (Phase B Round 6, May 2026)** dispatches on K_{a,b}-style disjoint-anchor 2D grids (Cm₂ at ~36 s vs §7.5 formula at ~55 s); Cm₃ still pending the pair-orbit-aware extension | O(rows × cols × Bell(W)² × poly²)                      |
| 6.10 | [Cell-Quotient Tree DP](06_6_cell_quotient_tree_dp.md)        | Cell-decomposable graphs with arbitrary TREE quotient (n cells, n−1 junctions, no cycles); building block for hybrid cycle-close + tree-DP-per-leaf | O(n × Bell(W)² × poly²); per-cell compression gives 50× on Cm₃ pattern M_2 |
