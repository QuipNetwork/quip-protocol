...
mod tests {
    ...
    fn insert_keeps_top_k_and_rejects_never_viable() {
        ...
        let ids: Vec<u8> = s.candidates.iter().map(|c| c.job_id[0]).collect();
    ...
    fn due_at_returns_best_arrived_unsubmitted() {
        ...
        assert_eq!(s.due_at(125).map(|c| c.job_id[0]), Some(1));
    ...
    fn summary_serializes_annotations() {
        let mut s = stash_with(vec![-50_000, -48_000, -46_000], 100, 10, 4);
        s.insert(cand(0xab, -49_000));
        let v: serde_json::Value =
            serde_json::from_str(&serde_json::to_string(&s.summary()).unwrap()).unwrap();
        assert_eq!(v["generation"], 1);
        assert_eq!(v["candidates"][0]["job_id"], "ab");
        assert_eq!(v["candidates"][0]["viability_block"], 110);
        assert_eq!(v["candidates"][0]["submitted"], false);
    }
}
