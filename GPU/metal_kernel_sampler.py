...
class UnifiedMetalSampler:
    ...
    def _compile_kernels(self):
        ...
        try:
            ...
            kernel_names = [
                ...
                "metropolis_sequential",
                "reduce_energies_2d",
                "pt_swap_adjacent",
                "compute_swap_stats",
                "compute_graph_coloring",
                "compute_adaptive_sweeps",
    ...
    def sample_ising(self, h: Dict[int, float], J: Dict[tuple, float],
                    ...
                    T_min: float = 0.155, T_max: float = 5.77,
                    sample_interval: Optional[int] = None,
                    ladder_target_acceptance: float = 0.30,
                    adapt_every: int = 2,
                    stats_window: int = 4,
                    max_replicas: Optional[int] = None,
                    enable_color_updates: bool = False,
                    use_sequential_updates: bool = False,
                    final_T_fraction: float = 0.031) -> dimod.SampleSet:
        """GPU-only pipeline: build CSR, choose replicas, make temperatures, partition reads, and sample — all on GPU.

        Hybrid PT/SA Parameters:
            T_min, T_max: Match D-Wave SA default range [0.155, 5.77]
            final_T_fraction: Final temperature as fraction of initial (0.031 matches D-Wave SA final temps)
                             Geometric cooling applied after each interval: T *= cooling_factor
            use_sequential_updates: If True, use sequential single-spin updates (matches CPU SA exactly)
                                   If False, use color-parallel updates (faster but different dynamics)
        """
        ...
        try:
            ...
            # Hybrid PT/SA: Compute geometric cooling factor
            num_intervals = max(1, int(num_sweeps) // interval)
            cooling_factor = final_T_fraction ** (1.0 / num_intervals)
            cooling_factor_buf = self._create_buffer(np.array([cooling_factor], dtype=np.float32), "cooling_factor")
            self.logger.debug(f"[UnifiedMetal] Hybrid PT/SA: geometric cooling over {num_intervals} intervals, "
                            f"final_T_fraction={final_T_fraction:.4f}, cooling_factor={cooling_factor:.6f}")
            ...
            flip_counts_prev_buf = self._create_buffer(np.zeros(num_reps, dtype=np.int32), "flip_counts_prev")  # Phase 3
            per_replica_sweeps_buf = self._create_buffer(np.full(num_reps, interval, dtype=np.int32), "per_replica_sweeps")  # Phase 3
            ...
            # D-Wave optimization: delta energy arrays [num_replicas * N]
            delta_energies_buf = self._create_buffer(np.zeros(num_reps * N, dtype=np.int32), "delta_energies")
            ...
            while remaining > 0:
                ...
                # Phase 3: Compute adaptive sweep allocation + Hybrid PT/SA cooling
                encAS = cbI.computeCommandEncoder()
                encAS.setComputePipelineState_(self._kernels["compute_adaptive_sweeps"])
                encAS.setBuffer_offset_atIndex_(num_reps_buf, 0, 0)
                encAS.setBuffer_offset_atIndex_(sweeps_buf_reusable, 0, 1)  # Total sweep budget
                encAS.setBuffer_offset_atIndex_(flip_counts_buf, 0, 2)
                encAS.setBuffer_offset_atIndex_(per_replica_sweeps_buf, 0, 3)
                encAS.setBuffer_offset_atIndex_(flip_counts_prev_buf, 0, 4)
                encAS.setBuffer_offset_atIndex_(temps_buf, 0, 5)             # Hybrid PT/SA
                encAS.setBuffer_offset_atIndex_(cooling_factor_buf, 0, 6)    # Hybrid PT/SA
                encAS.dispatchThreadgroups_threadsPerThreadgroup_(
                    Metal.MTLSize(width=1, height=1, depth=1),
                    Metal.MTLSize(width=1, height=1, depth=1)
                )
                encAS.endEncoding()

                # Phase 2+3: Metropolis updates (color-parallel OR sequential)
                enc2u = cbI.computeCommandEncoder()

                if use_sequential_updates:
                    # Sequential single-spin updates (matches CPU SA exactly)
                    enc2u.setComputePipelineState_(self._kernels["metropolis_sequential"])
                    ...
                    enc2u.setBuffer_offset_atIndex_(per_replica_sweeps_buf, 0, 6)
                    enc2u.setBuffer_offset_atIndex_(base_seed_buf, 0, 7)
                    enc2u.setBuffer_offset_atIndex_(thread_buffers_buf, 0, 8)  # IN-PLACE updates
                    enc2u.setBuffer_offset_atIndex_(flip_counts_buf, 0, 9)
                    # Sequential is single-threaded per replica
                    enc2u.dispatchThreadgroups_threadsPerThreadgroup_(
                        Metal.MTLSize(width=num_reps, height=1, depth=1),
                        Metal.MTLSize(width=1, height=1, depth=1)
                    )
                else:
                    # Color-parallel updates (faster but different dynamics)
                    ...
                    enc2u.setBuffer_offset_atIndex_(per_replica_sweeps_buf, 0, 8)
                    ...
                    enc2u.setBuffer_offset_atIndex_(delta_energies_buf, 0, 12)
                    # Debug counters
                    enc2u.setBuffer_offset_atIndex_(flip_counts_buf, 0, 13)
                    enc2u.setBuffer_offset_atIndex_(pos_counts_buf, 0, 14)
                    enc2u.setBuffer_offset_atIndex_(neg_counts_buf, 0, 15)
                    enc2u.setBuffer_offset_atIndex_(dbg_energy_buf, 0, 16)
