...
// WITH D-WAVE OPTIMIZATION: delta energy precomputation
kernel void metropolis_color_phase(
    ...
    // Parameters
    device const int* per_replica_sweeps [[buffer(8)]],  // [num_replicas] sweeps per replica (Phase 3)
    ...
    // D-Wave optimization: delta energy arrays
    device int* delta_energies [[buffer(12)]],        // [num_replicas * N]
    ...
    device int8_t* src = &thread_buffers_src[rid * n];
    device int8_t* dst = &thread_buffers_dst[rid * n];
    device int* delta_energy = &delta_energies[rid * n];

    int sweeps = per_replica_sweeps[rid];  // Phase 3: per-replica sweep budget
    int n_colors = *num_colors;

    // D-Wave optimization: threshold to skip impossible flips
    // At temperature T, probability of accepting ΔE > threshold is < 1e-10
    float threshold = 22.18f / beta;  // ln(1e10) ≈ 23.03, but D-Wave uses 22.18
    ...
    for (int sweep = 0; sweep < sweeps; ++sweep) {
        ...
        for (int color_idx = 0; color_idx < n_colors; ++color_idx) {
            ...
            // D-Wave optimization: precompute delta energies for this color
            // This is valid because all spins in this color are independent (no shared neighbors)
            for (int tile = 0; tile < n; tile += int(tg_size.x)) {
                int sid = tile + int(tid3_tg.x);
                if (sid < n && node_colors[sid] == current_color) {
                    ...
                    for (int p = start; p < end; ++p) {
                        ...
                        int8_t Jij = csr_J_vals[p];
                        int8_t neighbor_spin = src[j];
                        de += Jij * (new_spin - old_spin) * neighbor_spin;
                    }
                    delta_energy[sid] = de;
                }
                threadgroup_barrier(mem_flags::mem_device);
            ...
            // Update all spins with this color in parallel using precomputed delta energies
            for (int tile = 0; tile < n; tile += int(tg_size.x)) {
                int sid = tile + int(tid3_tg.x);
                if (sid < n && node_colors[sid] == current_color) {
                    int8_t old_spin = src[sid];
                    ...
                    // D-Wave optimization: use precomputed delta energy
                    int de = delta_energy[sid];

                    // D-Wave optimization: skip if delta energy too large
                    // If ΔE >= threshold, probability of acceptance is < 1e-10
                    if (de >= threshold) {
                        dst[sid] = old_spin;  // No flip
                        continue;
                    }
...
// 14. Adaptive sweep allocation (Phase 3) + Temperature Cooling (Hybrid PT/SA)
kernel void compute_adaptive_sweeps(
    device const int* num_replicas [[buffer(0)]],           // [1]
    device const int* total_sweeps [[buffer(1)]],           // [1] total sweep budget
    device const atomic_int* flip_counts [[buffer(2)]],     // [num_replicas] flips since last call
    device int* per_replica_sweeps [[buffer(3)]],           // [num_replicas] output: sweeps per replica
    device atomic_int* flip_counts_prev [[buffer(4)]],      // [num_replicas] previous flip counts for delta
    device float* temperatures [[buffer(5)]],               // [num_replicas] Hybrid PT/SA cooling
    device const float* cooling_factor [[buffer(6)]],       // [1] geometric cooling factor per interval
    uint tid [[thread_position_in_grid]]
) {
    if (tid != 0) return;  // Single-threaded

    int n_reps = *num_replicas;
    int budget = *total_sweeps;

    // Compute flip deltas
    int flip_deltas[256];  // Max replicas
    int total_flips = 0;

    for (int i = 0; i < n_reps; ++i) {
        int current = atomic_load_explicit(&flip_counts[i], memory_order_relaxed);
        int prev = atomic_load_explicit(&flip_counts_prev[i], memory_order_relaxed);
        flip_deltas[i] = max(1, current - prev);  // Avoid division by zero
        total_flips += flip_deltas[i];
        atomic_store_explicit(&flip_counts_prev[i], current, memory_order_relaxed);
    }

    // Compute flip rates (normalized)
    float flip_rates[256];
    float avg_rate = float(total_flips) / float(n_reps);

    for (int i = 0; i < n_reps; ++i) {
        flip_rates[i] = float(flip_deltas[i]) / avg_rate;  // >1 = active, <1 = stuck
    }

    // Allocate sweeps inversely proportional to flip rate (stuck replicas get more)
    // Use smoothing factor to avoid thrashing
    float weights[256];
    float total_weight = 0.0f;
    float smoothing = 0.25f;  // Conservative: 75% uniform, 25% adaptive

    for (int i = 0; i < n_reps; ++i) {
        // Inverse flip rate: low flips = high weight
        float inv_rate = 1.0f / max(0.1f, flip_rates[i]);
        weights[i] = smoothing * inv_rate + (1.0f - smoothing) * 1.0f;  // Blend with uniform
    ...
    int allocated = 0;
    for (int i = 0; i < n_reps; ++i) {
        float fraction = weights[i] / total_weight;
        int sweeps = int(fraction * float(budget));
        sweeps = max(1, sweeps);  // At least 1 sweep per replica
        per_replica_sweeps[i] = sweeps;
        allocated += sweeps;
    }

    // Handle rounding: distribute remainder to stuck replicas
    int remainder = budget - allocated;
    if (remainder > 0) {
        for (int i = 0; i < n_reps && remainder > 0; ++i) {
            if (flip_rates[i] < 1.0f) {  // Stuck replica
                per_replica_sweeps[i]++;
                remainder--;
            }
        }
    }

    // Final safety: ensure total equals budget
    while (remainder < 0) {
        // Find replica with most sweeps and reduce by 1
        int max_idx = 0;
        int max_sweeps = per_replica_sweeps[0];
        for (int i = 1; i < n_reps; ++i) {
            if (per_replica_sweeps[i] > max_sweeps) {
                max_sweeps = per_replica_sweeps[i];
                max_idx = i;
            }
        }
        if (per_replica_sweeps[max_idx] > 1) {
            per_replica_sweeps[max_idx]--;
            remainder++;
        } else {
            break;
        }
    ...
    // Hybrid PT/SA: Apply geometric temperature cooling to ALL replicas
    // This happens AFTER adaptive sweep allocation, once per interval
    float cool = *cooling_factor;
    for (int i = 0; i < n_reps; ++i) {
        temperatures[i] *= cool;
    }
}

// 15. Sequential single-spin Metropolis (for testing vs color-parallel)
// This version does sweeps at CURRENT temperature (for use in cooling loop)
kernel void metropolis_sequential(
    ...
    device const float* temperatures [[buffer(4)]],
    device const int* num_replicas [[buffer(5)]],
    device const int* per_replica_sweeps [[buffer(6)]],  // Changed to match color-parallel API
    device const uint* base_seed [[buffer(7)]],
    device int8_t* states [[buffer(8)]],  // [num_replicas * N] - IN-PLACE updates
    device atomic_int* flip_counts [[buffer(9)]],
    uint2 tid [[thread_position_in_grid]]  // x=replica, y=unused
) {
    int rid = int(tid.x);
    int num_reps = *num_replicas;
    if (rid >= num_reps) return;

    int n = *N;
    int sweeps = per_replica_sweeps[rid];  // Get sweep count for this replica
    float beta = 1.0f / max(temperatures[rid], 1e-6f);

    device int8_t* state = &states[rid * n];

    // Sequential sweeps
    for (int sweep = 0; sweep < sweeps; ++sweep) {
        // Update spins in sequential order 0, 1, 2, ..., N-1
        for (int sid = 0; sid < n; ++sid) {
            int8_t old_spin = state[sid];
            int8_t new_spin = -old_spin;
            ...
            for (int p = start; p < end; ++p) {
                int j = csr_col_ind[p];
                int8_t Jij = csr_J_vals[p];
                int8_t neighbor_spin = state[j];  // Read CURRENT state (includes updates from earlier spins in this sweep!)
                delta_energy += Jij * (new_spin - old_spin) * neighbor_spin;
            }
            ...
            bool accept = (delta_energy <= 0);
            if (!accept) {
                uint s = *base_seed ^ uint((rid + 1) * 2654435761u) ^ uint((sid + 1) * 974238197u) ^ uint((sweep + 1) * 362437u);
                s ^= s << 13; s ^= s >> 17; s ^= s << 5;
                float r = float(s & 0x7FFFFFFFu) / 2147483647.0f;
                float prob = exp(-float(delta_energy) * beta);
                accept = (r < prob);
            ...
            if (accept) {
                state[sid] = new_spin;  // UPDATE IN-PLACE
                atomic_fetch_add_explicit(&flip_counts[rid], 1, memory_order_relaxed);
            }
        }
    }
