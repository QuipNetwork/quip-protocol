...
kernel void hierarchical_block_update(
    ...
    device uint8_t* flips_out [[buffer(8)]],
    uint2 thread_id [[thread_position_in_grid]]
) {
    uint chain_idx = thread_id.x;
    ...
    int8_t current_spin = spins[flat_idx];
    float field = local_fields[flat_idx];
    // Use per-spin random value from a full R×n buffer (avoids per-block uploads)
    float rand_val = random_decisions[chain_idx * n + spin_idx];

    // Metropolis acceptance for energy minimization
    float delta_e = 2.0f * float(current_spin) * field;
    bool accept = (delta_e < 0.0f) || (rand_val < exp(-beta * delta_e));

    // Record flips for this block (one byte per chain × block position)
    if (flips_out) {
        uint flip_idx = chain_idx * block_size + block_spin_idx;
        flips_out[flip_idx] = accept ? uint8_t(1) : uint8_t(0);
    }
...
// (Kept for compatibility; not used in the optimized per-chain path below.)
kernel void hierarchical_tensor_coupling_update(
    ...
    // NOTE: This legacy kernel requires old/new spin arrays; use the new deltas kernel below instead.
}
// Kernel 12: Incremental update from block flips without host roundtrips
kernel void update_fields_from_block_flips(
    device float* local_fields [[buffer(0)]],
    device const int8_t* spins [[buffer(1)]],
    device const uint8_t* flips_out [[buffer(2)]],
    device const uint* i_indices [[buffer(3)]],
    device const uint* j_indices [[buffer(4)]],
    device const float* j_values [[buffer(5)]],
    constant uint& R [[buffer(6)]],
    constant uint& block_start [[buffer(7)]],
    constant uint& block_size [[buffer(8)]],
    constant uint& n [[buffer(9)]],
    ...
    uint chain_idx = gid.x;
    uint block_spin_idx = gid.y;
    if (chain_idx >= R || block_spin_idx >= block_size) return;

    // Only act if this spin flipped
    uint flip_mask_idx = chain_idx * block_size + block_spin_idx;
    if (flips_out[flip_mask_idx] == 0) return;

    uint global_spin_idx = block_start + block_spin_idx;
    if (global_spin_idx >= n) return;

    // Compute spin delta (new - old) = 2 * new_spin (since new = -old)
    uint chain_offset = chain_idx * n;
    int8_t new_spin = spins[chain_offset + global_spin_idx];
    float spin_delta = 2.0f * float(new_spin);

    // Update neighbors' fields due to this flip
    for (uint c = 0; c < num_couplings; c++) {
        uint i = i_indices[c];
        uint j = j_indices[c];
        float coupling_val = j_values[c];
        if (i == global_spin_idx) {
            ...
            atomic_fetch_add_explicit((device atomic<float>*)&local_fields[neighbor_idx], field_delta, memory_order_relaxed);
        } else if (j == global_spin_idx) {
...
// Kernel 12b: Incremental update using precomputed neighbor lists (sparse, fast)
kernel void update_fields_from_block_flips_sparse(
    device float* local_fields [[buffer(0)]],
    device const int8_t* spins [[buffer(1)]],
    device const uint8_t* flips_out [[buffer(2)]],
    device const uint* neighbor_offsets [[buffer(3)]],
    device const uint* neighbor_indices [[buffer(4)]],
    device const float* neighbor_weights [[buffer(5)]],
    constant uint& R [[buffer(6)]],
    constant uint& block_start [[buffer(7)]],
    constant uint& block_size [[buffer(8)]],
    constant uint& n [[buffer(9)]],
    ...
    // delta = new - old = 2 * new_spin (since new = -old)
    uint chain_offset = chain_idx * n;
    int8_t new_spin = spins[chain_offset + global_spin_idx];
    float spin_delta = 2.0f * float(new_spin);

    // Use neighbor list bounds for this spin
    uint start = neighbor_offsets[global_spin_idx];
    uint end = neighbor_offsets[global_spin_idx + 1];

    for (uint idx = start; idx < end; idx++) {
        uint neighbor = neighbor_indices[idx];
        float weight = neighbor_weights[idx];
        uint neighbor_field_idx = chain_offset + neighbor;
        float field_delta = weight * spin_delta;
        atomic_fetch_add_explicit((device atomic<float>*)&local_fields[neighbor_field_idx], field_delta, memory_order_relaxed);
    }
}

// Kernel 12c: Fused block update + sparse neighbor field update (single dispatch)
kernel void hierarchical_block_update_fused(
    device int8_t* spins [[buffer(0)]],
    ...
    device uint8_t* flips_out [[buffer(11)]],
    uint2 gid [[thread_position_in_grid]]
) {
    uint chain_idx = gid.x;
    uint block_spin_idx = gid.y;
    ...
    uint chain_offset = chain_idx * n;
    uint flat_idx = chain_offset + spin_idx;
    ...
    float delta_e = 2.0f * float(current_spin) * field;
    float accept = exp(-beta * delta_e);
    bool do_flip = (delta_e <= 0.0f) || (rand_val < accept);
    flips_out[chain_idx * block_size + block_spin_idx] = do_flip ? 1 : 0;
    ...
    // Update neighbors' local fields using sparse adjacency
    float spin_delta = 2.0f * float(new_spin);
    uint start = neighbor_offsets[spin_idx];
    uint end = neighbor_offsets[spin_idx + 1];
    for (uint idx = start; idx < end; idx++) {
        uint neighbor = neighbor_indices[idx];
        float weight = neighbor_weights[idx];
        uint neighbor_field_idx = chain_offset + neighbor;
...
// Kernel 12d: Clear flip mask for current block range
kernel void clear_flip_mask_range(
    device uint8_t* flips_out [[buffer(0)]],
    constant uint& R [[buffer(1)]],
    constant uint& block_size [[buffer(2)]],
    constant uint& current_block_size [[buffer(3)]],
    uint2 gid [[thread_position_in_grid]]
) {
    uint chain_idx = gid.x;
    uint block_spin_idx = gid.y;
    if (chain_idx >= R || block_spin_idx >= current_block_size) return;
    flips_out[chain_idx * block_size + block_spin_idx] = 0;
}

// Kernel 12e: Sequential within-block fused update (Gauss-Seidel style)
kernel void hierarchical_block_update_fused_seq(
    device int8_t* spins [[buffer(0)]],
    device float* local_fields [[buffer(1)]],
    device const float* random_decisions [[buffer(2)]],
    device const uint* neighbor_offsets [[buffer(3)]],
    ...
    // Iterate spins in block sequentially per chain
    for (uint block_spin_idx = 0; block_spin_idx < block_size; ++block_spin_idx) {
        uint spin_idx = block_start + block_spin_idx;
        if (spin_idx >= n) break;
...
// Kernel 12: Incremental field updates using per-flip deltas (optimized per-chain)
kernel void incremental_field_update_deltas(
    device float* local_fields [[buffer(0)]],          // Input/Output: local field values (for ONE chain via buffer offset)
    device const uint* i_indices [[buffer(1)]],        // Input: coupling i-indices
    device const uint* j_indices [[buffer(2)]],        // Input: coupling j-indices
    device const float* j_values [[buffer(3)]],        // Input: coupling strengths
    device const uint* flipped_indices [[buffer(4)]],  // Input: indices of flipped spins
    device const float* spin_deltas [[buffer(5)]],     // Input: per-flip spin delta values (±2)
    constant uint& n [[buffer(6)]],                    // Input: total number of spins
    constant uint& num_couplings [[buffer(7)]],        // Input: number of coupling terms
    constant uint& num_flipped [[buffer(8)]],          // Input: number of flipped spins
    uint thread_id [[thread_position_in_grid]]
) {
    uint flipped_idx = thread_id;
    if (flipped_idx >= num_flipped) return;

    uint global_flipped_idx = flipped_indices[flipped_idx];
    if (global_flipped_idx >= n) return;

    float spin_delta = spin_deltas[flipped_idx];  // ±2 based on old/new spin change

    // Update neighbors' local fields for this chain
    for (uint c = 0; c < num_couplings; c++) {
        uint i = i_indices[c];
        uint j = j_indices[c];
        float coupling_val = j_values[c];
        ...
        if (i == global_flipped_idx) {
            // Update local field of spin j
            float field_delta = coupling_val * spin_delta;
            atomic_fetch_add_explicit((device atomic<float>*)&local_fields[j], field_delta, memory_order_relaxed);
        } else if (j == global_flipped_idx) {
            // Update local field of spin i
            float field_delta = coupling_val * spin_delta;
            atomic_fetch_add_explicit((device atomic<float>*)&local_fields[i], field_delta, memory_order_relaxed);
        }
    }
}

// Kernel 13: Sparse neighbor adjacency list update (placeholder)
kernel void sparse_incremental_field_update(
    ...
    // Not implemented yet; use incremental_field_update_deltas or the legacy kernel.
    return;
}
