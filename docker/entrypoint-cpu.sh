#!/bin/bash
# Entrypoint script for CPU miner Docker container
set -e
...
OUTPUT_FILE="${OUTPUT_FILE:-/output/cpu_${INSTANCE_ID}_${TIMESTAMP}.json}"
LOG_FILE="${OUTPUT_FILE%.json}.log"

echo "========================================" | tee -a "$LOG_FILE"
echo "Quip Protocol - CPU Mining Comparison" | tee -a "$LOG_FILE"
echo "========================================" | tee -a "$LOG_FILE"
...
if [ -n "$S3_BUCKET" ]; then
    ...
    aws s3 cp "$OUTPUT_FILE" "s3://$S3_BUCKET/cpu/" 2>&1 | tee -a "$LOG_FILE"
    aws s3 cp "$LOG_FILE" "s3://$S3_BUCKET/cpu/" 2>&1 | tee -a "$LOG_FILE"
fi
