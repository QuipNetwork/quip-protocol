...
def _chain_uses_hybrid_signature(iface: SubstrateInterface) -> bool:
    ...
    `MultiSignature` chains. The metadata layout (V14+) exposes types under
    `["V14"]["types"]["types"]` with a `path` list per type.
    """
    try:
        ...
        for t in types_list:
            ...
            if "HybridTxSignature" in path:
                return True
        return False
    except Exception as exc:  # noqa: BLE001 — metadata shape varies across versions
        logger.warning(
            "could not introspect HybridTxSignature in metadata: %s; "
            "defaulting to sr25519 mode",
            exc,
        )
        return False
...
class SubstrateFaucet:
    ...
    def _submit_hybrid_transfer(self, *, dest: str, amount: int) -> dict:
        ...
        # Mirror substrate-interface's submit-and-watch flow: subscribe,
        # return on first inblock/finalized status, raise on dropped/invalid.
        result: dict = {"extrinsic_hash": ext_hash, "block_hash": None}
