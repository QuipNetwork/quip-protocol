...
impl UtilGovernor {
    /// Start a util poller when `yielding` is true.
    ///
    /// Falls back to a silent no-op governor if sensors are unavailable
    /// (miner still runs; utilization stays 0 and throttle never fires).
    ...
    /// True when yielding and last sample exceeds 90%.
...
fn poll_loop(_device_index: u32, last: Arc<AtomicU32>, stop: Arc<AtomicBool>) {
    while !stop.load(Ordering::Relaxed) {
        last.store(query_iokit_gpu_utilization(), Ordering::Relaxed);
