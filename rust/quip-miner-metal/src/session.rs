//! Shared gRPC session loop for `quip-metal-sa` / `quip-metal-gibbs`.
//!
//! Modeled on `quip-miner-cuda` / `quip-mock-miner`: UDS bidi stream, Hello →
//! Welcome → Configure → Ready, job handling with Reject reasons, Status on
//! Ping/Cancel, clean drain on Shutdown / idle timeout.
//!
//! One process per GPU: `--device N` binds Metal device N and defaults
//! `--miner-id` to `metal-N` (matching `[metal.N]` config sections).
//!
//! On non-macOS the CLI still parses flags (`--capabilities`, `--check`);
//! session / GPU paths return [`ExitCode::EnvIncompatible`].

// On Linux CI the GPU session path is never entered; helpers stay compiled so
// the module structure matches macOS, but they are unused.
#![cfg_attr(
    not(target_os = "macos"),
...
use clap::Parser;
use quip_proto::v1::{
    ising_problem, miner_msg, IsingProblem, JobKind, MinerMsg, Reject, RejectReason, Status,
};
use quip_protocol::session::ExitCode;
use quip_protocol::wire::{decode_i32_le, WireError};
...
#[cfg(target_os = "macos")]
use crate::sampler::{sample_ising, SampleParams};
#[cfg(target_os = "macos")]
use quip_proto::v1::miner_service_client::MinerServiceClient;
#[cfg(target_os = "macos")]
use quip_proto::v1::{
    coord_msg, CoordMsg, Job, JobRequest, Ready, Result as JobResult, SamplerMeta, Solution,
};
#[cfg(target_os = "macos")]
use quip_protocol::session::{build_hello, SessionConfig, SessionError};
#[cfg(target_os = "macos")]
...
/// CLI surface shared by both Metal miner binaries.
#[derive(Parser, Debug)]
...
pub struct Cli {
    ...
    /// Metal device index (usually 0). Default 0 → miner id `metal-0`.
    #[arg(long, default_value_t = 0)]
    pub device: usize,
    /// Target GPU utilization ceiling percent (1–100). Used by IOKit governor.
    #[arg(long, default_value_t = 100)]
    pub utilization: u32,
    /// Yield to other GPU users when observed util exceeds 90%.
...
fn print_capabilities(id: AlgorithmIdentity) {
    println!(
        r#"{{"backend":"metal","algorithm":"{}","supported_kinds":["ISING_SAMPLE"],"max_nodes":{},"max_edges":{}}}"#,
...
async fn run_session(
    ...
    // MetalDevice is owned directly (not Arc'd) and never crosses a spawn
    // boundary: sample_ising runs synchronously inside this single
    // block_on future, so every GPU call stays on one OS thread for the
    // life of the process (see metal_device module docs for why).
...
fn map_err_to_exit(err: Box<dyn std::error::Error>) -> StdExitCode {
    ...
    eprintln!("quip-miner-metal fatal: {err}");
...
pub fn run_cli(identity: AlgorithmIdentity) -> StdExitCode {
    ...
    {
        let _ = (cli.device, cli.utilization, cli.yielding, identity);
        if cli.check {
            eprintln!("metal check failed: Metal is only available on macOS");
            return StdExitCode::from(ExitCode::EnvIncompatible as u8);
        }
        if cli.quip_coordinator.is_none() {
            eprintln!("error: --quip-coordinator required for session mode");
            return StdExitCode::from(ExitCode::ConfigInvalid as u8);
        }
        eprintln!("quip-miner-metal fatal: Metal is only available on macOS");
    ...
    {
        if cli.check {
            match MetalDevice::check(cli.device) {
                Ok(()) => return StdExitCode::SUCCESS,
                Err(e) => {
                    eprintln!("metal check failed (device {}): {e}", cli.device);
                    return StdExitCode::from(ExitCode::EnvIncompatible as u8);
                }
            }
        }
        ...
        let miner_id = cli
            .miner_id
            .unwrap_or_else(|| format!("metal-{}", cli.device));

        // Own MetalDevice directly (not Arc): keeps every GPU call on this one
        // thread for the process lifetime (see metal_device.rs module docs).
        let device = match MetalDevice::open(cli.device) {
            Ok(d) => d,
            Err(e) => {
                eprintln!("failed to open Metal device {}: {e}", cli.device);
                return StdExitCode::from(ExitCode::EnvIncompatible as u8);
            }
        };
