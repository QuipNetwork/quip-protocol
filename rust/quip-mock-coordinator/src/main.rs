...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bin_path = args.next().ok_or("usage: quip-mock-coordinator <miner-bin> <unix://socket>")?;
    let socket = args.next().ok_or("usage: quip-mock-coordinator <miner-bin> <unix://socket>")?;
